...
const headerNav: navigationItem[] = [
	{
		type: '/',
		name: 'Головна'
	},
...
const Header: React.FC= () => {
	const { totalCount, products } = useSelector(selectCart)


	const [visibleBurger, setVisibleBurger] = React.useState<string>('menu');
		...
		<header className="header">
			...
			<nav className={`header__nav ${visibleBurger}`}>
				<div className="header__icons" >
					...
					<Link onClick={handelToIconCart} to={'cart'} className='header__icon cart-icon'>
						{products.length
							?
				...
				<ul>
					...
					<li
						...
						className="header__nav-item"
					>
						<a href="tel:+380960167568" rel="nofollow" className='link__nav'>+380960167568</a>
					</li>
				</ul>
			</nav>
